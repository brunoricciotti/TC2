...
def dibujar_plantilla_bp(ax,wc,ws,a_c,a_s,a_b):
    ...
    ax.set_xlim(ws[0]/10,ws[1]*10)
    ax.set_ylim(t_min_plantilla,t_max_plantilla)
    ax.set_xlabel('Frecuencia [rad / s]')
    ax.set_ylabel('Amplificación [dB]')
...
def dibujar_plantilla_lp(ax,wc,ws,a_c,a_s,a_b):
    # generate sample
    w_min = 0
    w_max = ws*100
    t_max_b = -a_b
    ...
    t_min_plantilla = t_max_s-10
    t_max_plantilla = t_max_b+10
    ax.fill([w_min, w_min,  wc  ,  wc ], [t_max_b,t_min_c, t_min_c, t_max_b], alpha=0.2, color="green") # pass
    ax.fill([ws   , ws   , w_max,w_max], [t_min_plantilla, t_max_s, t_max_s, t_min_plantilla],  alpha=0.2, color="green") # stop
    ax.set_xlim(wc/10,ws*10)
    ax.set_ylim(t_min_plantilla,t_max_plantilla)
    ax.set_xlabel('Frecuencia [rad / s]')
    ax.set_ylabel('Amplificación [dB]')
    ax.grid(which='both', axis='both')
